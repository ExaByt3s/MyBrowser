...
bool HTTPClient::get(HTTPRequest& req,
	HTTPResponse& rep,
	std::string& strData)
{
	do {
		if (!parseUrl(req)) { break; }
		HTTP_REQ_TYPE type = HTTP_GET;
		HTTPSession session;
		if (!sendHeader(type, session, req)) { break; }
		if (!readHeader(session, rep)) { break; }
		if (!readBody(session, rep, strData)) { break; }
		return true;
	} while (false);
	return false;
}

bool HTTPClient::post(HTTPRequest& req,
	HTTPResponse& rep,
	...
	std::string& strResult)
{
	do {
		...
		HTTPSession session;
		if (!sendHeader(type, session, req)) { break; }
		if (!sendBody(session, strData)) { break; }
		if (!readHeader(session, rep)) { break; }
...
bool HTTPClient::startPost(HTTPSession& session,
	HTTPRequest& req)
{
	do {
		if (!parseUrl(req)) { break; }
...
bool HTTPClient::postData(HTTPSession& session,
	const std::string& data)
{
	return sendBody(session, data);
}

void HTTPClient::stopPost(HTTPSession& session,
	HTTPResponse& rep,
	std::string& strResult)
{
...
bool HTTPClient::startGet(HTTPSession& session,
	HTTPResponse& rep,
	HTTPRequest& req)
{
	do {
...
{
	do {
		if (req.url.length() <= strlen("http://")) { break; }
		...
		if (colon && result && colon > result) { break; }
		if (result && colon) {
...
void HTTPClient::makeHeader(HTTP_REQ_TYPE& type,
	HTTPRequest& req,
	std::string &strHead)
{
...
void HTTPClient::makeMultipartBody(HTTPMultipart& multipart,
	std::string& strBody)
{
	std::ostringstream oss;
	for (std::map<uint32_t,
		MultiPart>::iterator it = multipart.m_multiPartMap.begin();
		it != multipart.m_multiPartMap.end(); ++it) {
		...
		for (std::map<std::string, std::string>::iterator st = part.m_headMap.begin();
			st != part.m_headMap.end();
			++st) {
...
bool HTTPClient::sendHeader(HTTP_REQ_TYPE& type,
	HTTPSession& session,
	HTTPRequest& req)
{
	do {
		...
		if (!session.connect(ip, port)) { break; }
		...
		do {
			if (rep.bChunked) {
				...
				if (ret > 0) {
					...
					if (rep.uReadLength >= rep.uContentLength) { break; }
				}
			}
		} while (ret >= 0);
