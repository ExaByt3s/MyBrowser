...
// 硬盘序列号
char HardDriveSerialNumber[1024]{ 0x00 };
// 硬盘型号
char HardDriveModelNumber[1024]{ 0x00 };
// 路由MAC
char RouteMac[18]{ 0x00 };
// MAC地址
char mac[64]{ 0x00 };
// CPU序列号
char strCpuId[32]{ 0x00 };
// 制造商信息
char cVendorID[13]{ 0x00 };
// IP地址
char ipAddr[1024]{ 0x00 };
...
CStdString CPUID::GetCpuVID()
{
    // 执行cpuid指令，
	// 使用输入参数 eax = 0
    Executecpuid(0);
    // 复制前四个字符到数组
    memcpy(cVendorID, &m_ebx, 4);
    // 复制中间四个字符到数组
    memcpy(cVendorID + 4, &m_edx, 4);
    // 复制最后四个字符到数组
    memcpy(cVendorID + 8, &m_ecx, 4);

    // 以string的形式返回
    return CStdString(cVendorID);
}

CStdString CPUID::GetCpuSerialNumber()
{
	SerialNumber serial;
    // 执行cpuid，参数为 eax = 1
    Executecpuid(1);
    ...
    return CStdString(strCpuId);
}

PIP_ADAPTER_INFO pinfo;
unsigned long len = 0;

CStdString CPUID::getMAC()
{
    NCB ncb;
	...
	CStdString strMac(mac);
	return strMac;
}
// 
// 获取网卡MAC地址
// 
CStdString CPUID::GetMacInfo()
{
	if (pinfo != NULL) {
		// pinfo指针已经指向next了,
		// 不能再简单的释放
...
CStdString CPUID::ParseData()
{
	CStdString strMac;

	while (pinfo != NULL) {
		CStdString strPinfoDesc(pinfo->Description);
...
{	
	...
	try
	{
		...
		CStdString VendorID((LPCSTR)vendor_id);
...
// 获取硬盘序列号
CStdString CPUID::getHDSerialNum()
{
    long id = getHardDriveComputerID();
    CStdString strHdSerialNum(HardDriveSerialNumber);
...
// 获取硬盘型号
CStdString CPUID::getHDModelNum()
{
    long id = getHardDriveComputerID();

    CStdString strModel(HardDriveModelNumber);

    return strModel;
}

// 获取路由Mac
CStdString CPUID::getRouteMac()
{
	getAdapterInfo();

    CStdString strRouteMac(RouteMac);
