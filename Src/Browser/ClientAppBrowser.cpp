...
namespace Browser
{
		...
		CefRefPtr<CefCommandLine> command_line)
	{
		if (process_type.empty()) {
			...
			// cef在LoadUrl建立渲染进程时，会查找子进程的路径，
			// 可能会引发一个bug导致IDE在debug状态时卡死
			// 这里指定好子进程路径就可以了

			// 但是使用sandbox的话，不允许使用另外的子进程;
			// 不使用sandbox的话，第一次加载flash插件时会弹出个命令提示行，
			// 这是cef的bug。flash与子进程二者不可兼得
		    ...
		    // ============ flash在线测试 ============
			// http://webcast.cechina.cn/Help/TestFlv.aspx
		    // http://ssfw.njfy.gov.cn/ssfwzx/ext/flash/flash.jsp

			// 指定Flash插件
			command_line->AppendSwitchWithValue("ppapi-flash-version", "32.0.0.156");
			command_line->AppendSwitchWithValue("ppapi-flash-path", "Plugins\\PepperFlash\\pepflashplayer.dll");
#endif
			// 同一个域下的使用同一个渲染进程
