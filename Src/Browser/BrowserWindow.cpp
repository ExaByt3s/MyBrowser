...
namespace Browser
{
		...
		CefRefPtr<CefRequestContext> request_context)
	{
		...
		CefBrowserHost::CreateBrowser(
			window_info, 
			m_ClientHandler,
			url.empty() ? m_ClientHandler->StartupUrl() : url, 
			settings, 
			request_context);
	}

	void BrowserWindow::GetPopupConfig(
	...
	void BrowserWindow::ShowBrowser(
		int nBrowserId, 
		int x, 
		int y,
		size_t width,
	...
	void BrowserWindow::OnDragLinkDrop(CefRefPtr<CefBrowser> browser, CefString linkUrl)
	{
		DCHECK(CefCurrentlyOn(TID_UI));
	...
	void BrowserWindow::OnSetAddress(
	...
	void BrowserWindow::OnSetFullscreen(
		CefRefPtr<CefBrowser> browser, 
		bool fullscreen)
	{
		DCHECK(CefCurrentlyOn(TID_UI));
	...
	void BrowserWindow::OnSetLoadingState(
		CefRefPtr<CefBrowser> browser,
		bool isLoading,
		bool canGoBack,
		bool canGoForward)
