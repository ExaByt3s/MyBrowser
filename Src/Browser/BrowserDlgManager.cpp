...
namespace Browser
{
	...
	void BrowserDlgManager::CloseAllWindows(bool force)
	{
		if (!CefCurrentlyOn(TID_UI)) {
			...
			CefPostTask(TID_UI,
				CefCreateClosureTask(base::Bind(&BrowserDlgManager::CloseAllWindows, 
					base::Unretained(this), 
					force)));
			return;
		}

		if (m_BrowserDlgSet.empty()) { return; }
		...
		if (!CefCurrentlyOn(TID_UI)) {
			...
			CefPostTask(TID_UI, 
				CefCreateClosureTask(
					base::Bind(&BrowserDlgManager::OnBrowserDlgCreated, 
						base::Unretained(this), 
						pDlg)));
			return;
		}
	...
	void BrowserDlgManager::OnBrowserDlgDestroyed(BrowserDlg* pDlg)
	{
		...
		if (m_bTerminateWhenAllWindowsClosed &&
			m_BrowserDlgSet.empty()) {
