...
namespace Browser
{
		...
		CefRefPtr<CefMenuModel> model)
	{
		...
		model->AddItem(CLIENT_ID_REFRESH, CefString(L"刷新当前页面"));
		if(GLOBALS::strShowDevTools.CompareNoCase(_T("1")) == 0) {
			model->AddItem(CLIENT_ID_SHOW_DEVTOOLS, CefString(L"显示开发者工具"));
		    model->AddItem(CLIENT_ID_CLOSE_DEVTOOLS, CefString(L"关闭开发者工具"));
		}
		// model->AddItem(CLIENT_ID_LOGO, CefString(L"显示LOGO"));
		// model->AddItem(CLIENT_ID_BINDING, CefString(L"Binding测试"));
	}
	...
	bool ClientHandler::OnJSDialog(CefRefPtr<CefBrowser> browser,
		...
		if (dialog_type == JSDIALOGTYPE_ALERT) {
			// 普通提示框
			MessageBox(NULL, CStdString(message_text.c_str()), _T("提示"), MB_OK);
			suppress_message = true;
			return false;
		} else if (dialog_type == JSDIALOGTYPE_CONFIRM) {
			// 询问框
			int result = ::MessageBox(NULL, CStdString(message_text.c_str()), _T("请确认"), MB_OKCANCEL);
			if(result == IDOK) {
			...
			// 输入框，不支持
			MessageBox(NULL, _T("输入框类型JS弹窗暂时不支持！"), _T("提示"), MB_OK);
			suppress_message = true;
			return false;
		}
	...
	bool ClientHandler::OnBeforeUnloadDialog(
		...
		CStdString msg = _T("离开此网站？\r\n\r\n系统可能不会保存您所做的更改。");
		int result = ::MessageBox(NULL, msg, _T("请确认"), MB_OKCANCEL);
		if (result == IDOK) {
			callback->Continue(true, "");
		} else {
			callback->Continue(false, "");
		...
		CefRefPtr<CefDownloadItemCallback> callback)
	{
		...
		if (download_item->IsComplete())
		{
			ClientRunner::Alert(
				browser,
				CefString(_T("文件【")).ToString()
				    + download_item->GetFullPath().ToString() 
				    + CefString(_T("】 下载完成！")).ToString()
			);
		}
	}
		...
		CefDragHandler::DragOperationsMask mask)
	{
		...
		//	// MessageBox(NULL, CStdString(fileNames.at(0).c_str()), _T("信息"), MB_OK);
		// }
