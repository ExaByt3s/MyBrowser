...
namespace Browser
{
	...
	{
		const char arrTestOrigin[] = "http://tests/";
		...
		void SetupResourceManager(CefRefPtr<CefResourceManager> resource_manager) {
			...
			const std::string& test_origin = arrTestOrigin;
		...
		namespace Binding
		{
			const char arrTestUrl[] = "http://tests/binding";
			const char arrTestMessageName[] = "BindingTest";
			...
			public:
					...
					CefRefPtr<Callback> callback) OVERRIDE
				{
					...
					if (url.find(arrTestUrl) != 0) {
						return false;
					}

					const std::string& message_name = request;
					if (message_name.find(arrTestMessageName) == 0) {
						// Reverse the string and return.
						std::string result = message_name.substr(sizeof(arrTestMessageName));
